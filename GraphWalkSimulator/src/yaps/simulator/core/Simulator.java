...
import viewer.control.TurnAgentUtil;
import yaps.graph.Edge;
import yaps.graph.Graph;
import yaps.patrolling_metrics.VisitsList;

/**
 * The parameter indicates if the simulation will "jump over" turns in which the
 * agents are just on the edges, moving between nodes. <br>
 * <br>
 * If set to "true", the simulator will only dispatch events in turn in which
 * there is at least one agent on a node. If set to false, the simulator will do
 * the default simulation turn by turn, dispatching all events applicable in
 * each turn.
 */
public class Simulator implements Runnable {
	...
	private VisitsList visits; // salvar aqui ou em arquivo de log (oferecer
								// escolha)

	// simulation info
	private int currentTurn;
	private AgentTeamInfo teamInfo;
	private boolean symmetricalGraph;

	//Object responsible for the GUI
	private TMAPGraphDrawUtil gui;

	//Agents Turns information.
	...
	public void setGraph(Graph map) {
		this.graph = map;
		// TODO: verificar se � simetrico
		this.symmetricalGraph = true;
	}
	...
	// TODO: falta outros gets, falta os comentarios dos set ?
	...
	public void run() {
		...
		while (currentTurn < this.totalTime) {
			...
			// => Como o tempo e discreta, para o algoritmo, ele sera o turno
			// *currTime+1*
			// => A ideia e que o simulador esta pedindo as acoes que serao
			// realizadas no intervalo [currTime; currTime+1)
			...
			if (!agUtil.isEmpty()) {
				...
				gui.setTextTurn("TURN " + currentTurn + " ----------> "
						+ teamInfo);

				//Move agents on gui.
				for (int i = 0; i < agUtil.size(); i++) {
					gui.getAgentUtil()
							.moveAgent(agUtil.get(i).getAgentId(),
									agUtil.get(i).getCurrent(),
									agUtil.get(i).getNext());
				}

				//Loop to wait agents movement on gui.
				while (gui.getAgentUtil().getAgents()
						.get(agUtil.get(agUtil.size() - 1).getAgentId())
						.isAgentMoving()) {
					// Wait agent movement.
				}
	...
	private void moveAgentToNext(int agentId) {
		...
		//If gui is seted this step create a object with agents movement information.
		...
		if (agent.inNode()) {
			if (next >= 0 && next != current) {
				if (graph.existsEdge(current, next)) {
					...
					// current, next);
					agent.setEdgePosition(current, next, 1); // dist=1,
																// assumindo o
																// resultado
																// ap�s 1 ciclo
				...
				visits.addVisit(currentTurn, current, agentId); // agente
																// revisita o no
			...
			if (next == agent.destination) {
				...
				if (this.symmetricalGraph) {
					...
					throw new Error(
							"Agents can only change direction in symmetrical graphs");
		...
		if (checkArrival) {
			if (agent.distance == edge.getLength()) {
				...
				teamInfo.nextActions[agentId] = -1; // if no other action is set
													// by the algorithm, the
													// agent will stop until
													// next turn
	...
	public void onSimulationStart(Graph graph, AgentPosition[] initialInfo,
		...
		for (int ag = 0; ag < numAgents; ag++) {
			...
			if (this.agentPath[ag].get(0) != initialInfo[ag].getCurrentNode()) {
				throw new Error("Path of agent " + ag
						+ " starts with wrong start node: "
						+ this.agentPath[ag].get(0));
			}
		}
	}
	...
	public void onTurn(int nextTurn, AgentTeamInfo team) {
		...
		for (int id = 0; id < numAgents; id++) {
			...
			if (agentPosition.inNode()) {
				...
				if (this.agentPathIndex[id] < this.agentPath[id].size()) {
					nextNode = this.agentPath[id].get(this.agentPathIndex[id]);
				} else {
					nextNode = agentPosition.getCurrentNode(); // stand still
																// (it is not
																// necessary to
																// set the
																// "target node"
